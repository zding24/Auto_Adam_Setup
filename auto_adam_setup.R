    ...
    for (i in 1:length(df_list)){
      df_all <- left_join(df_all, df_list[[i]], by = keys, 
                          suffix = c(ifelse(i == 1, 
                                            ...
                                            paste0('.', tolower(df_vec[i-1]))), 
                                     paste0('.', tolower(df_vec[i]))))
      print(paste0('Merge dataset: ', df_vec[i], ' by ', paste(keys, collapse = ', ')))
    }
    return(df_all)
  }
}
...
load_data <- function(adam_path, sdtm_path, df){
  ...
  if (file.exists(df_path)){
    ...
    print(paste0('Identify dataset: ', df, ' as: ', flag))
    print(paste0('File path: ', df_path))
    return(df_data)
  } else if (tolower(df) == 'adsl'){
    ...
    if (file.exists(df_alt_path)){
      ...
      print(paste0('Identify dataset: ', df, ' as: ', flag))
      print('Original file was not found. Read adsl_i instead')
      print(paste0('File path: ', df_path))
      return(df_data)
    } else{
      print(paste0('Identify dataset: ', df, ' as: ', flag))
